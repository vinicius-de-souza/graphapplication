...
void No::inserirAresta(int alvo_id, float peso){
    ...
    if(this->primeira_aresta != nullptr){
        ...
        Aresta* Aresta = new class Aresta(alvo_id);
        Aresta->setpeso(peso);
...
// Other methods
    ...
    int No::removeAresta(int id, bool directed, No* target_node){
        ...
        if(this->buscaAresta(id)){
            ...
            while(aux->getTargetId() != id){
                ...
                previous = aux;
                aux = aux->getNextAresta();

            }
            // Keeping the integrity of the Aresta list
            if(previous != nullptr)
                previous->setNextAresta(aux->getNextAresta());

            else
                this->primeira_aresta = aux->getNextAresta();

            if(aux == this->ultima_aresta)
                this->ultima_aresta = previous;

            if(aux->getNextAresta() == this->ultima_aresta)
                this->ultima_aresta = aux->getNextAresta();
    ...
    bool No::procuraAresta(int target_id){
        // Verifies whether there are at least one Aresta in the node
        if(this->primeira_aresta != nullptr){
            // Searching for a specific Aresta of target id equal to target id
            for(Aresta* aux = this->primeira_aresta; aux != nullptr; aux = aux->getProxAresta())
    ...
    Aresta* No::temArestaEntre(int target_id)
    {
        ...
        {
            if(auxAresta->getAlvoId() == target_id)
                return auxAresta;
        }
        return nullptr;
    }
